...
(ns infra.federation-test
  (:require [clojure.test :refer [deftest is testing]]
            [infra.detectives.matrix :as matrix]))
...
(deftest client-api-test
  (is (= #{"the Matrix client API is unreachable"}
         (summaries (matrix/detect-client-api {"host" "synapse.infra.run"
  ...
  (is (empty? (matrix/detect-client-api {"host" "synapse.infra.run"
                                         "client-versions" {:reachable true :status 200}}))))

(deftest federation-api-test
  (is (= #{"the federation API is unreachable"}
         (summaries (matrix/detect-federation-api {"host" "synapse.infra.run"
  ...
  (is (empty? (matrix/detect-federation-api {"host" "synapse.infra.run"
...
(deftest wellknown-test
  (testing "absent delegation is fine (SRV/direct setups exist)"
    (is (empty? (matrix/detect-wellknown {"domain" "infra.run"
  ...
  (testing "served-but-broken delegation warns"
    (is (= #{".well-known/matrix/server is served but has no m.server field"}
           (summaries (matrix/detect-wellknown {"domain" "infra.run"
                                                "wellknown-server" {:reachable true :status 200
                                                                    :json {:something "else"}}})))))
  (testing "valid delegation is silent"
    (is (empty? (matrix/detect-wellknown {"domain" "infra.run"
                                          "wellknown-server" {:reachable true :status 200
                                                              :json {(keyword "m.server") "synapse.infra.run:443"}}})))))
