...
(ns infra.spec-test
  ...
  (:require [clojure.edn :as edn]
            ...
            [infra.detectives.pods :as pod-detectives]
            [infra.effect :as effect]
            [infra.spec :as spec]))

(deftest finding-schema-test
  (testing "a well-formed finding conforms; malformed ones do not"
    (is (spec/valid? :infra.spec/finding {:severity :warning :component "ns/x" :summary "trouble"}))
    (is (spec/valid? :infra.spec/finding {:severity :info :component "x" :summary "y" :hint "do this"}))
    (is (not (spec/valid? :infra.spec/finding {:severity :oops :component "x" :summary "y"})))
    (is (not (spec/valid? :infra.spec/finding {:severity :info :component "x" :summary ""})))
    (is (not (spec/valid? :infra.spec/finding {:severity :info :summary "no component"}))))
  (testing "real detectives emit conforming findings"
    (let [findings (concat
      ...
      (is (every? #(spec/valid? :infra.spec/finding %) findings)
          (str "non-conforming: " (remove #(spec/valid? :infra.spec/finding %) findings))))))

(deftest effect-schema-test
  (testing "every effect constructor produces a conforming effect"
    (is (spec/valid? :infra.spec/effect (effect/kubectl "ctx" "cordon" "n")))
    (is (spec/valid? :infra.spec/effect (effect/kubectl-stdin "ctx" "{}" "replace" "--raw" "/x" "-f" "-")))
    (is (spec/valid? :infra.spec/effect (effect/ssh {:host "h"} "ceph" "-s")))
    (is (spec/valid? :infra.spec/effect (effect/cmd "flux" "reconcile")))
    (is (spec/valid? :infra.spec/effect (effect/note "hello"))))
  (testing "a plan of effects conforms; an unknown op does not"
    (is (spec/valid? :infra.spec/plan (effect/plan (effect/kubectl "c" "get" "pods")
                                                   (effect/note "done"))))
    (is (not (spec/valid? :infra.spec/effect [:teleport "somewhere"])))
    (is (not (spec/valid? :infra.spec/plan [[:kubectl "c" "x"] [:teleport "y"]])))))
