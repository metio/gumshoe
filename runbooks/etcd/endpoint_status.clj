  ...
  (let [context (kubectl/current-context)
    ...
    (if (nil? target)
      ...
      (let [{:keys [namespace name]} (kubectl/split-namespace-name target)]
        ...
        (etcd/etcdctl! context namespace name "endpoint" "status" "--write-out=table")))))

(runbook/execute!
 {:description "Shows the status of the etcd endpoint of one etcd pod"
  :options options
