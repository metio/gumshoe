...
(ns runbooks.etcd.endpoint-health
  "Checks the health of the etcd endpoint of one etcd pod."
  ...
  (let [context (kubectl/current-context)
    ...
    (if (nil? target)
      ...
      (let [{:keys [namespace name]} (kubectl/split-namespace-name target)]
        ...
        (etcd/etcdctl! context namespace name "endpoint" "health" "--write-out=table")))))

(runbook/execute!
 {:description "Checks the health of the etcd endpoint of one etcd pod"
  :options options
